...
1. **Byte-identical** — for all 3 fixtures, ``analyze()`` (default halee_ramone)
   ...
   unchanged, and the golden regression still reports 93/93 (the P-035 corpus — see conftest.py). The mechanism: the
...
3. **Liveness (load-bearing)** — a synthetic profile that weights
   ...
   ``dense_chorus_with_loops``; a sabotage that DROPS the term or breaks its
   threading FAILS that liveness while byte-identical stays green
   (P-016/P-029). A term hardcoded to a constant is caught by the
   value-discrimination guards, not by liveness (the direction check reads
   the score from the same output a constant would poison).
