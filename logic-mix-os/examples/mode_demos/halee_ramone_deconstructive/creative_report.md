...
**Search mode:** `deconstructive` — remove, mute, chop, filter, simplify

**Mode reach (profile-authored):** favors `subtractive_drop`, `loop_deconstruct`; suppresses `width_bloom`, `drum_room_bloom` — capped at `medium` risk
...
_Mode fork: suppressed `width_bloom`, `drum_room_bloom`; favored `subtractive_drop`._
...
_Mode fork: favored `subtractive_drop`, `loop_deconstruct`._
